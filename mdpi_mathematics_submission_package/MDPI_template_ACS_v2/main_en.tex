...
\AtBeginDocument{%
  ...
  \hypersetup{
    ...
    pdfauthor={Ezyn SEGNANE, Younes OMMANE, Khalil EL WALED, Mohamedou CHEIKH TOURAD, Mariam MOUADILI, Diego H. PELUFFO-ORDONEZ, Beddi Mohamed ABDELLAHI},
...
% Author E (Mouadili): ORCID not publicly registered — icon omitted
% Author F (Peluffo-Ordóñez): ORCID confirmed
\newcommand{\orcidauthorF}{0000-0002-9045-6997}
% Author G (Beddi Mohamed Abdellahi): ORCID not publicly registered — icon omitted

\Author{Ezyn~SEGNANE$^{1,\ast}$\orcidA{}, Younes~OMMANE$^{2}$\orcidB{}, Khalil~EL~WALED$^{1}$\orcidC{}, Mohamedou~CHEIKH~TOURAD$^{1}$\orcidD{}, Mariam~MOUADILI$^{2}$, Diego~H.~PELUFFO-ORD\'O\~NEZ$^{3}$\orcidF{} and Beddi~Mohamed~ABDELLAHI$^{4}$}
\AuthorNames{Ezyn SEGNANE, Younes OMMANE, Khalil EL WALED, Mohamedou CHEIKH TOURAD, Mariam MOUADILI, Diego H. PELUFFO-ORD\'O\~NEZ and Beddi Mohamed ABDELLAHI}
\AuthorCitation{SEGNANE, E.; OMMANE, Y.; EL WALED, K.; CHEIKH TOURAD, M.; MOUADILI, M.; PELUFFO-ORD\'O\~NEZ, D.H.; ABDELLAHI, B.M.}
...
\item[\textbf{Group D} (exploratory, 20 runs).] Augmentation ablation: standard (\texttt{aug}, reference) vs.\ no-augmentation (\texttt{noaug}), 10 seeds each.
...
Group~E establishes two key facts. First, the ECG signal is indispensable: the meta-only variant (E1) collapses to $0.6966$ macro-AUC, far below any acceptable clinical threshold. Second, the specific fusion mechanism does not drive macro-AUC differences at this scale: all six mechanism variants (E2--E7) achieve $0.9287$--$0.9293$, within the noise of the standard configuration ($p{>}0.27$ in all cases). This is notable because E2 (simple concatenation) and E3 (additive injection) achieve performance comparable to the GLU gate, suggesting that the gate's added complexity is not necessary to capture the metadata signal at this effect magnitude. The E8 result ($0.5046$ macro-AUC) is a negative control confirming that training without metadata prevents the model from learning to use it: the metadata pathway must be active at training time.

\subsection{Q6 --- generalisation: fold 10 is a conservatively difficult split}
Group~F shows that the four alternative fold pairs (folds 2, 3, 7, 8 as test folds) all achieve macro-AUC in the range $[0.9400, 0.9445]$, substantially above fold~10 ($0.9289$). This $0.011$--$0.016$ gap is consistent with fold-level difficulty variation in stratified k-fold settings and confirms that the confirmatory Group~A results, conditioned on fold~10, likely \emph{understate} the typical performance of EZNX-ATLAS-A. The conservative nature of fold~10 should be noted when positioning our results relative to benchmarks evaluated on averaged or best-of-fold metrics.
...
\begin{enumerate}[leftmargin=1.4em]
    \item \textbf{Evaluation scope and fold difficulty.} The confirmatory Group~A results are conditioned on fold~10, which Group~F identifies as lying below the inter-fold macro-AUC range $[0.9400, 0.9445]$ by approximately $0.011$--$0.016$. This indicates that fold~10 is a more challenging split and that the reported gains are conservative estimates. No external cohort evaluation has been performed, limiting generalisation across institutions, acquisition devices, demographic distributions, and temporal shifts.
...
\item \textbf{Quality-gated fusion} with zero-initialised residual injection remains stable under simulated missingness, losing only $\approx 0.0014$ macro-AUC under full anthropometric masking, converging to the demographics-only baseline.
...
\item \textbf{Group~D} (augmentation ablation, 20 runs) finds that removing light stochastic augmentation causes a nominally significant macro-AUC drop ($\Delta{=}{-}0.0014$, $p{=}0.0195$), indicating that augmentation is not a neutral design choice.
\item \textbf{Group~E} (architectural ablations, 80 runs) confirms that the ECG signal is indispensable (meta-only AUC $0.6966$) and that metadata must be present during training (E8 trainmask AUC $0.5046$); the specific fusion mechanism does not materially affect macro-AUC.
\item \textbf{Group~F} (multi-split check, 20 runs) shows that fold~10 lies below the inter-fold range $[0.9400, 0.9445]$ from four alternative splits, confirming that Group~A results are conservatively estimated.
\end{enumerate}
...
\vspace{6pt}
\noindent\textbf{Use of Generative AI:} During the preparation of this submission, the authors used Claude (Anthropic) to audit the reference list for dataset and terminology consistency, to verify manuscript compliance with journal formatting requirements, and to correct grammar, spelling, and punctuation errors in the manuscript text. The authors reviewed all AI-generated suggestions and take full responsibility for the content of this publication.
