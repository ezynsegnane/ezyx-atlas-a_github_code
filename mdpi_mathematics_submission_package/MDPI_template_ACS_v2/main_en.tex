...
% ---- Abstract ----------------------------------------------
\abstract{We present EZNX-ATLAS-A, a 3.95\,M-parameter dual-branch model for paired multi-seed quantification of the incremental contribution of patient metadata to 12-lead ECG superclass classification on PTB-XL 1.0.3. The architecture couples a residual ECG encoder with a demographic-and-anthropometric branch, an explicit metadata-availability score ($q_{\mathrm{meta}}$), and GLU-style late fusion. A pre-declared \textbf{250-run} campaign across six experimental groups (A--F) provides the confirmatory evidence via Group~A (three metadata variants $\times$ \textbf{20 seeds}, test fold~10, paired Wilcoxon signed-rank tests with Benjamini--Hochberg correction over the \textbf{pre-specified 3-test family}). Two statistically significant gains are confirmed: demographic variables (age, sex) yield $\Delta{=}{+}0.0007$ macro-AUC (95\% paired CI $[0.0002, 0.0011]$, $p_{\mathrm{BH}}{\approx}0.009$), localised to NORM and MI; full metadata yield $\Delta{=}{+}0.0019$ macro-AUC (95\% paired CI $[0.0014, 0.0023]$, $p_{\mathrm{BH}}{<}0.001$), concentrated in NORM, MI, and STTC. CD and HYP show no statistically significant metadata effect. Exploratory sensitivity analyses (Groups~B--F) indicate no detectable sensitivity to fusion-capacity or loss-balance hyperparameters, and show that fold~10 yields lower absolute demo+anthro macro-AUC than four alternative folds, consistent with fold-level difficulty variation. The absolute gains are small ($\Delta{<}0.002$) and should not be interpreted as clinical utility without external validation. These findings provide a statistically principled, reproducible baseline for multimodal ECG evaluation, demonstrating the value of a pre-specified paired seed-matched measurement framework with explicit multiplicative-correction control.}
...
\item \textbf{Group~F} (multi-split check, 20 runs, descriptive only) shows that absolute \texttt{demo+anthro} macro-AUC varied across four alternative splits (range $[0.9400, 0.9445]$), with fold~10 below this range; within-fold paired comparisons of Group~A are unaffected. Note that the metadata normalization index was built from folds~1--8 and the alternative test folds overlap with this set, so Group~F absolute values should be treated as descriptive only.
\end{enumerate}
