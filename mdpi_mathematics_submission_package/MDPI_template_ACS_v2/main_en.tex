...
\noindent\footnotesize\textit{Notes:} $\pm$ values for paired differences are SEM (standard error of the mean over 20 paired differences). For the least-significant test (\texttt{demo}$-$\texttt{none}), $p_{\mathrm{BH}} = p_{\mathrm{raw}}$ because this test occupies rank 3/3 in the sorted family ($p_{\mathrm{BH}} = p_{\mathrm{raw}} \times 3/3 = p_{\mathrm{raw}}$).
\end{table}
