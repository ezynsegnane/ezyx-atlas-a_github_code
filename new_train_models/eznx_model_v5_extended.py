# =============================================================================
# eznx_model_v5_extended.py
# Extended architecture with configurable GLU gate hidden dimension.
#
# Addresses review item:
#   H7 - GLU width sensitivity sweep (gate_hidden_dim in {512, 1152, 2048}).
#
# All other components are identical to eznx_model_v5.py.
# When gate_hidden_dim == fuse_dim (== 1152), the model is architecturally
# identical to the original and produces numerically identical results for a
# given seed.
# =============================================================================
...
class TemporalStatPool(nn.Module):
    """Statistical pooling: mean, std, max, min along the time axis."""
...
class ResBlock1D(nn.Module):
    """Residual 1-D convolution block."""
...
class TSBackbone1D_v5(nn.Module):
    """Residual 1-D backbone for 12-lead ECG (base=64 → out_dim=1024)."""

    def __init__(self, in_ch=12, base=64):
        super().__init__()
        self.conv_init = nn.Conv1d(in_ch, base, 7, stride=2, padding=3, bias=False)
        ...
        self.out_dim   = base * 4 * 4   # 1024 for base=64
...
class MetaMLP_v5(nn.Module):
    """Two-hidden-layer MLP for structured metadata."""

    def __init__(self, in_dim, hid=128, out_dim=128, dropout=0.1):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(in_dim, hid), nn.ReLU(), nn.BatchNorm1d(hid), nn.Dropout(dropout),
            nn.Linear(hid, hid),   nn.ReLU(), nn.BatchNorm1d(hid), nn.Dropout(dropout),
...
class EZNX_ATLAS_A_v5(nn.Module):
    """
    EZNX-ATLAS-A dual-branch multimodal model — extended.

    Extension over eznx_model_v5.py:
      gate_hidden_dim (int | None): hidden width of the GLU gate MLP.
        - None  → use fuse_dim (1152); architecturally identical to original.
        - 512   → narrow gate (H7 sensitivity run).
        - 2048  → wide gate  (H7 sensitivity run).

    Gate architecture:
        Linear(fuse_dim -> gate_hidden_dim) -> ReLU
        -> Linear(gate_hidden_dim -> fuse_dim) -> Sigmoid

    fuse_dim = ts.out_dim + meta_out = 1024 + 128 = 1152 (default settings).
    """
    ...
    def __init__(
        ...
        meta_out=128,
        meta_dropout_p=0.1,
        gate_hidden_dim=None,      # H7: None => fuse_dim (1152)
    ):
        super().__init__()

        if meta_dim % 2 != 0 or meta_dim < 4:
            raise ValueError("meta_dim must be an even integer >= 4.")

        self.meta_dim       = int(meta_dim)
        self.meta_value_dim = self.meta_dim // 2          # 8
        ...
        self.anthro_value_dim = self.meta_value_dim - self.demo_value_dim   # 6
        ...
        self.anthro_encoder = MetaMLP_v5(self.anthro_value_dim * 2, hid=96, out_dim=64,      dropout=0.10)
        # Input to meta_fuse: 64 + 64 + 1 (demo_q) + 1 (anthro_q) = 130
        self.meta_fuse = nn.Sequential(
            nn.Linear(130, self.meta_hid), nn.ReLU(), nn.BatchNorm1d(self.meta_hid), nn.Dropout(0.10),
            nn.Linear(self.meta_hid, meta_out), nn.ReLU(),
        )
        self.meta_dropout_p = float(meta_dropout_p)

        # --- Fusion ---
        fuse_dim = self.ts.out_dim + meta_out    # 1024 + 128 = 1152
        self.fuse_dim = fuse_dim
        _ghid = gate_hidden_dim if gate_hidden_dim is not None else fuse_dim
        self.gate_hidden_dim = int(_ghid)

        # GLU gate with configurable hidden width (H7)
        self.gate = nn.Sequential(
            nn.Linear(fuse_dim, self.gate_hidden_dim),
            nn.ReLU(),
            nn.Linear(self.gate_hidden_dim, fuse_dim),
            nn.Sigmoid(),
        )
        ...
        # Zero-initialised residual injection
    ...
    def forward(self, x_ts, x_meta, meta_present_mask=None):
        ...
        if use_meta:
            ...
            if x_meta.size(1) != self.meta_value_dim:
                raise ValueError(
                    f"Expected x_meta with {self.meta_value_dim} features, "
                    f"got {x_meta.size(1)}."
                )
            if meta_present_mask.size(1) != self.meta_value_dim:
                raise ValueError("meta_present_mask must match x_meta feature dim.")
